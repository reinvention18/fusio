...
const CHATS_FILE = join(DATA_DIR, 'seo-chat-sessions.json');
const CHATS_BACKUP = join(DATA_DIR, 'seo-chat-sessions.backup.json');
const BACKUP_DIR = join(DATA_DIR, 'seo-chat-sessions-backups');
...
function rollingBackup(reason: string, keepLast = 24): void {
  try {
    ...
    const dest = join(BACKUP_DIR, `seo-chat-sessions.${iso}.${reason}.json`);
    copyFileSync(CHATS_FILE, dest);

    // Prune — keep most recent N files
    const files = readdirSync(BACKUP_DIR)
      .filter(f => f.startsWith('seo-chat-sessions.') && f.endsWith('.json'))
